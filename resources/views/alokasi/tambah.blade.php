...
@section('content')
    ...
    <div class="container-fluid">
    	<div class="card shadow mb-4">
	    	...
	    	<form action="{{ asset(route('alokasi.create', [], false)) }}" method="POST">
	    	@csrf
		    ...
		    <div class="card-body">
	            ...
	            <div class="form-group {{$errors->has('nama_petugas') ? 'has-error' : ''}}">
				    ...
				    <select name="nama_petugas" class="form-control" id="nama_petugas">
				      	...
				      	@foreach($data_petugas as $petugas)
				      		<option value="{{ $petugas->id }}" {{ (old('nama_petugas') == $petugas->id) ? 'selected' : '' }}>{{$petugas->nama}}</option>
				      	@endforeach
				    </select>
				...
				<div id="jab" style="display: none" class="form-group {{$errors->has('jabatan') ? 'has-error' : ''}}">
					<label for="jabatan" >Jabatan</label>
				    <input name="jabatan" class="form-control" id="jabatan" readonly >
				...
				<div class="form-group {{$errors->has('status') ? 'has-error' : ''}}">
				    ...
				    <select name="status" class="form-control" id="status" >
				      	...
				      	<option value="PCL" {{ (old('status') == 'PCL') ? 'selected' : '' }}>PCL</option>
				      	<option value="PML" {{ (old('status') == 'PML') ? 'selected' : '' }}>PML</option>				      	
				    </select>
				...
				<div class="form-group {{$errors->has('kode') ? 'has-error' : ''}}">
	              ...
	              <input name="kode" type="text" class="form-control" placeholder="Kode Petugas" value="{{ old('nks') }}">
	              @if ($errors->has('kode'))
	                <span class="text-danger">{{$errors->first('kode')}}</span>
	              @endif
	            </div>

				<div id="atasan" style="display: none" class="form-group {{$errors->has('nama_atasan') ? 'has-error' : ''}}">
				    ...
				    <select name="nama_atasan" class="form-control" id="nama_atasan">
				      	...
				      	@foreach($data_petugas as $petugas)
				      		<option value="{{ $petugas->id }}" {{ (old('nama_atasan') == $petugas->id) ? 'selected' : '' }}>{{$petugas->nama}}</option>
				      	@endforeach
				    </select>
				...
				{{-- <div class="form-group {{$errors->has('kode_pml') ? 'has-error' : ''}}">
	              <label>Kode Pengawas</label>
	              <input name="kode_pml" type="text" class="form-control" placeholder="Kode Pengawas" value="{{ old('nks') }}">
	              @if ($errors->has('kode_pml'))
	                <span class="text-danger">{{$errors->first('kode_pml')}}</span>
	              @endif
	            </div> --}}
			...
			<div class="card-footer py-3">
				  <button type="submit" class="btn btn-primary"><i class="fa fa-check-circle"></i> Tambah Alokasi Petugas</button>
			</div>
