...
@section('content')
	...
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
	    <h1 class="h3 mb-0 text-gray-800">Input Dokumen</h1>
    </div>
    	...
    	<div class="card shadow mb-4">
    		<div class="card-header py-3">    			
		      	<h6 class="m-0 font-weight-bold  card-title">
		      		<a href="{{ asset(route('dokumen.refresh', ['kode_kegiatan'=> $dokumen->kegiatan_id, 'kode_nks'=> $dokumen->nks], false)) }}"><i class="fas fa-chevron-left"></i></a>
		      		Input Dokumen
		  		</h6>
		    </div>    	

	    	<form action="{{ asset(route('dokumen.update', ['page'=> $page], false)) }}" method="POST">
	    	@csrf
		    ...
		    <div class="card-body">
		    	...
		    	<div class="form-group">
	              ...
	              <select name="kegiatan" class="form-control">
	                ...
	                @foreach($data_kegiatan as $kegiatan)
	                  <option value="{{ $kegiatan->id }}" {{ ($dokumen->kegiatan_id == $kegiatan->id) ? 'selected' : '' }}>{{$kegiatan->nama_keg}}&nbsp;{{$kegiatan->periode}}&nbsp;{{$kegiatan->tahun}}</option>                
	                @endforeach
	              </select>
	            ...
	            <div class="form-group {{$errors->has('nks') ? 'has-error' : ''}}">
				    ...
				    <select name="nks" class="form-control" id="nks">
				      	...
				      	@foreach($data_dsbs as $dsbs)
				      		<option value="{{ $dsbs['nks'] }}" {{ ($dokumen->nks == $dsbs['nks']) ? 'selected' : '' }}>{{ $dsbs['nks'] }}</option>
				      	@endforeach
				    </select>
				    @if ($errors->has('nks'))
	            ...
	            <div class="form-group {{$errors->has('nus') ? 'has-error' : ''}}">
				    ...
				    <select name="nus" class="form-control" id="nus">
				      	...
				      	@for ($i = 1; $i < 11; $i++)
				      		<option value="{{ $i }}" {{ ($dokumen->nus == $i) ? 'selected' : '' }}>{{ $i }}</option>
				      	@endfor
				    </select>
				    @if ($errors->has('nus'))
		    	...
		    	<input type="hidden" name="kegiatan" value="{{$dokumen->kegiatan_id}}">
		    	<input type="hidden" name="provinsi" value="{{$kode_provinsi}}">
		    	<input type="hidden" name="kabupaten" value="{{$kode_kabupaten}}">
		    	{{-- <input type="hidden" name="nks" value="{{$dokumen->nks}}"> --}}
		    	{{-- <input type="hidden" name="nus" value="{{$dokumen->nus}}"> --}}
		    	<input type="hidden" name="page" value="{{$page}}">

				<div class="form-group {{$errors->has('p1') ? 'has-error' : ''}}">
	              <label>P1</label>
	              <input name="p1" type="text" class="form-control" placeholder="" value="{{ $dokumen->p1 }}">
		            <span class="text-xs">
				      Jumlah ART yang Sedang Bersekolah (P612=2)
				    </span>
	              @if ($errors->has('p1'))
	            ...
	            <div class="form-group {{$errors->has('p2') ? 'has-error' : ''}}">
	              <label>P2</label>
	              <input name="p2" type="text" class="form-control" placeholder="" value="{{ $dokumen->p2 }}">
		            <span class="text-xs">
				      Jumlah Simcard Aktif (Blok VIII Jumlah R803 s.d. R806)
				    </span>
	              @if ($errors->has('p2'))
	            ...
	            <div class="form-group {{$errors->has('p3') ? 'has-error' : ''}}">
	              <label>P3</label>
	              <input name="p3" type="text" class="form-control" placeholder="" value="{{ $dokumen->p3 }}">
		            <span class="text-xs">
				      Jumlah ART yang Memiliki BPJS (Blok XI R1101 yang dilingkari A)
				    </span>
	              @if ($errors->has('p3'))
	            ...
	            <div class="form-group {{$errors->has('status') ? 'has-error' : ''}}">
	            	<label>Status Pemasukan Dokumen</label>
		            <div class="form-check">
					  <input class="form-check-input" value="Selesai" type="radio" name="status" id="status_selesai" {{ ($dokumen->status == 'Selesai') ? 'checked' : '' }}>
					  <label class="form-check-label" for="status_selesai">
					    Selesai
					  </label>
					</div>
					<div class="form-check">
					  <input class="form-check-input" value="Belum" type="radio" name="status" id="status_belum" {{ ($dokumen->status == 'Belum') ? 'checked' : '' }}>
			...
			<div class="card-footer py-3">
				  <button type="submit" class="btn btn-primary"><i class="fa fa-check-circle"></i> Input Pemasukan Dokumen</button>
			</div>
...
<script>
	...
	base_url="{{asset('')}}";	
		...
		/**** JQuery Dinamic Dropdown Pilih Dokumen *******/
		...
		$(document).ready(function() {			
    		...
    		$('select[name="nus"]').on('change', function() {
    		 	...
    		 	var url = base_url+'dokumen/'+kegiatan+'/'+nks+'/'+nus+'/'+page+'/edit';
